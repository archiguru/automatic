...
sudo apt-get update -y && sudo apt-get install -y apt-transport-https ca-certificates curl software-properties-common build-essential gcc gcc-11-locales autoconf automake make libtool flex bison git bzr libgd-tools expect lrzsz zip unzip bzip2 zsh screen wget rsync lsof telnet tree htop vim net-tools universal-ctags gnupg2 jq cron socat gdb chrony gsmartcontrol hwloc psmisc debian-keyring ufw
cd $HOME && wget https://go.dev/dl/go1.22.1.linux-arm64.tar.gz
sudo rm -rf /usr/local/go && tar -C /usr/local -xzf go1.22.1.linux-arm64.tar.gz
sudo ufw disable
